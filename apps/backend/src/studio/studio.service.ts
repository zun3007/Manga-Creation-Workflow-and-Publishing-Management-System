...
export class StudioService implements OnModuleInit {
  ...
  async savePageVersion(
    ...
    const cur = await this.db.queryOne<{ current_version: number }>(
      `SELECT current_version FROM \`Page\` WHERE page_id = ?`,
      [pageId],
    );
    ...
    await this.db.insert(
      `INSERT INTO \`Page_Version\` (page_id, version_number, image_url, uploaded_by_user_id, upload_note)
       VALUES (?, ?, ?, ?, ?)`,
      [pageId, next, imageUrl, userId, note ?? null],
    );
    await this.db.query(
      `UPDATE \`Page\` SET current_version = ?, page_status = 'IN_PROGRESS' WHERE page_id = ?`,
      [next, pageId],
    );
  ...
  async getDoc(userId: number, pageId: number) {
    ...
    const row = await this.db.queryOne<{ manifest_json: unknown }>(
      `SELECT manifest_json FROM \`Studio_Document\` WHERE page_id = ?`,
      [pageId],
    );
    if (!row) return null;
    return typeof row.manifest_json === 'string'
