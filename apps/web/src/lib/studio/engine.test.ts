...
describe('StudioEngine', () => {
  ...
  it('duplicateLayer can be undone and redone', async () => {
    const wasm = await InkforgeWasm.load(wasmBytes as BufferSource);
    const doc = createDocument({ width: 4, height: 4, background: 'transparent' });
    const eng = new StudioEngine(doc, wasm);
    eng.bucketFill(0, 0, { r:100,g:50,b:25,a:255 }, 0);
    const srcId = eng.doc.activeLayerId!;
    const beforeCount = eng.doc.layers.length;
    eng.duplicateLayer(srcId);
    expect(eng.doc.layers).toHaveLength(beforeCount + 1);
    expect(eng.getBuffer(eng.doc.activeLayerId!)[0]).toBe(100);
    eng.undo();
    expect(eng.doc.layers).toHaveLength(beforeCount);
    expect(eng.doc.activeLayerId).toBe(srcId);
    eng.redo();
    expect(eng.doc.layers).toHaveLength(beforeCount + 1);
    expect(eng.getBuffer(eng.doc.activeLayerId!)[0]).toBe(100);
  });
  it('mergeDown can be undone and redone', async () => {
    const wasm = await InkforgeWasm.load(wasmBytes as BufferSource);
    let doc = createDocument({ width: 4, height: 4, background: 'transparent' });
    doc = addLayer(doc, 'raster', 'top');
    const eng = new StudioEngine(doc, wasm);
    const topId = eng.doc.activeLayerId!;
    const bottomId = eng.doc.layers[0].id;
    eng.bucketFill(1, 1, { r:255,g:0,b:0,a:255 }, 0);
    eng.setActiveLayer(bottomId);
    eng.bucketFill(0, 0, { r:0,g:255,b:0,a:255 }, 0);
    eng.setActiveLayer(topId);
    expect(eng.doc.layers).toHaveLength(2);
    eng.mergeDown(topId);
    expect(eng.doc.layers).toHaveLength(1);
    eng.undo();
  ...
  it('flattenImage can be undone and redone', async () => {
    const wasm = await InkforgeWasm.load(wasmBytes as BufferSource);
    let doc = createDocument({ width: 4, height: 4, background: 'white' });
    doc = addLayer(doc, 'raster', 'top');
    const eng = new StudioEngine(doc, wasm);
  ...
  it('paste can be undone and redone', async () => {
    const wasm = await InkforgeWasm.load(wasmBytes as BufferSource);
    const doc = createDocument({ width: 4, height: 4, background: 'transparent' });
    const eng = new StudioEngine(doc, wasm);
    eng.bucketFill(0, 0, { r:123,g:45,b:67,a:255 }, 0);
    eng.copySelection();
    expect(eng.doc.layers).toHaveLength(1);
    eng.paste();
    expect(eng.doc.layers).toHaveLength(2);
    const pastedBuffer = eng.getBuffer(eng.doc.activeLayerId!);
    expect(pastedBuffer[0]).toBe(123);
    eng.undo();
    expect(eng.doc.layers).toHaveLength(1);
    eng.redo();
    expect(eng.doc.layers).toHaveLength(2);
    const redoBuffer = eng.getBuffer(eng.doc.activeLayerId!);
    expect(redoBuffer[0]).toBe(123);
  });
});
