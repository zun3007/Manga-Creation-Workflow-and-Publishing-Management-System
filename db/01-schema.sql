...
CREATE TABLE `User` (
    ...
    `password_hash` VARCHAR(255),                       -- nullable: OAuth-only users have no local password
    ...
    `updated_at` DATETIME NOT NULL DEFAULT current_timestamp ON UPDATE current_timestamp,
     ...
     UNIQUE KEY `uq_user_google` (`google_id`),
...
CREATE TABLE `Series_Proposal` (
    ...
    `sample_manuscript_name` VARCHAR(255),
    `sample_manuscript_uploaded_at` DATETIME,
    `review_note` TEXT,
    `decision_note` TEXT,
...
CREATE TABLE `Chapter` (
    ...
    `deadline` DATETIME,
    `chapter_status` ENUM('DRAFT','IN_PROGRESS','READY_FOR_EDITOR_REVIEW','EDITOR_APPROVED','BOARD_APPROVED','PUBLISHED') NOT NULL DEFAULT 'DRAFT',
