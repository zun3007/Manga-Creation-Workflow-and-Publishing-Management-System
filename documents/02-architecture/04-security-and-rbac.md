...
**Purpose:** Describe authentication flows, JWT issuance, role-based authorization guards, the 5-role permission matrix, data-level ownership checks, and security hardening state of the manga platform.
...
6. [Input & Transport Security](#6-input--transport-security)
7. [Client-Side & Privacy](#7-client-side--privacy)
8. [Secrets & Configuration](#8-secrets--configuration)
9. [Hardening Backlog](#9-hardening-backlog)
...
The token is signed using `@nestjs/jwt` with the secret loaded from `JWT_SECRET` environment variable (default 'dev-secret' in dev mode).
Token expiration is managed by the JWT library's default (typically 1 hour); refresh tokens are not yet implemented.
...
### Flow A: Local Email + Password Login
...
sequenceDiagram
  ...
  Web->>Web: Extract token from URL; localStorage.setItem('token', accessToken)
...
### File Upload
- **POST /api/uploads**: Multer middleware validates and stores files to the local `./uploads/` directory.
- Filenames are randomized using UUID to prevent path traversal or name collisions.
- Uploaded files are served as static assets at `/uploads/{filename}` with read-only HTTP semantics (no DELETE/MOVE).
- File type and size constraints are enforced in the controller (details in the API reference).
...
### Upload Storage
- File uploads are stored on the API server's local filesystem (`./uploads/`).
- In production, this does not scale across multiple API instances and lacks redundancy.
- **Backlog**: Move uploads to object storage (AWS S3, Google Cloud Storage, or MinIO) with pre-signed URLs and versioning.
